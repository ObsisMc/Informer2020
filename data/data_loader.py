...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        # Obsismc: why subtract seq_len? maybe only pre_len is considered into datasets
        border1s = [0, num_train - self.seq_len, len(df_raw) - num_test - self.seq_len]
        border2s = [num_train, num_train + num_vali, len(df_raw)]
        border1 = border1s[self.set_type]  # get corresponding low bound
        border2 = border2s[self.set_type]  # get corresponding high bound

        # Obsismc: get needed data
        if self.features == 'M' or self.features == 'MS':
            cols_data = df_raw.columns[1:]
            df_data = df_raw[cols_data]
        elif self.features == 'S':
        ...
        if self.scale:
            ...
            # Obsismc: normalize all values in columns (for each time series)
            self.scaler.fit(train_data.values)
            data = self.scaler.transform(df_data.values)
        else:
            data = df_data.values
        ...
        df_stamp = df_raw[['date']][border1:border2]
        df_stamp['date'] = pd.to_datetime(df_stamp.date)
        data_stamp = time_features(df_stamp, timeenc=self.timeenc, freq=self.freq)  # Obsismc: default timeenc 1, freq h

        self.data_x = data[border1:border2]
        if self.inverse:
            self.data_y = df_data.values[border1:border2]
        else:
            self.data_y = data[border1:border2]  # Obsismc: if not inverse, data_y == data_x
        self.data_stamp = data_stamp

    def __getitem__(self, index):
        # Obsismc:
        # [s_begin, s_end] for encoder? len is "seq_len"
